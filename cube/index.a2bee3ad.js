...
*/ class CubeTime {
    ...
    * @returns {CubeTime}
    */ constructor(num1, plus_two1, dnf1){
        this.num = num1;
        this.plus_two = plus_two1;
        this.dnf = dnf1;
    }
    get getNum() {
        return this.num;
 ...
 */ function text_parse(solves) {
    ...
    for(let i = 0; i < lines.length; i++){
        line = lines[i];
        if (!time_list_flag) {
            if (line == "Time List:") time_list_flag = true;
            else if (line.startsWith("1.")) time_list_flag = true;
            else continue;
        }
        if (line == "" || line == "\n") continue;
        time = line.split("   ")[0];
        ...
        if (time.includes("[")) {
            ...
            if (open_bracket_index !== -1 && closed_bracket_index !== -1 && closed_bracket_index > open_bracket_index) {
 ...
 * @returns {Number[]}
 */ function times_to_time_nums(times1) {
    let time_nums = [];
    for(let i = 0; i < times1.length; i++){
        let time1 = times1[i];
        time_nums.push(time1.getNum);
    }
    return time_nums;
}
/**
 ...
 * @param {CubeTime[]} times 
 * @returns {Number}
 */ function average_of_n(times1) {
    let sum = 0;
    let num_of_times = times1.length;
    // fix calculation for small length lists.
    if (num_of_times == 0) return null;
    if (num_of_times <= 2) {
        temp_times = times_to_time_nums(times1);
    ...
    for(let i = 0; i < times1.length; i++){
        ...
        if (time1.getDNF) {
            num_dnfs++;
            DNFs.push(time1.getNum);
            times_to_remove.push(time1);
        }
    }
    if (num_dnfs > num_trim) return null;
    for(let i = 0; i < times_to_remove.length; i++){
        let time_to_remove = times_to_remove[i];
        let index_to_remove = times1.indexOf(time_to_remove);
        if (index_to_remove !== -1) times1.splice(index_to_remove, 1);
    }
    times1 = times_to_time_nums(times1);
    // sort by value
    times1.sort(function(a, b) {
        return a - b;
    });
    times1 = times1.concat(DNFs);
    times1 = times1.splice(num_trim);
    times1 = times1.splice(0, times1.length - num_trim);
    sum = times1.reduce((partialSum, a)=>partialSum + a, 0);
 ...
 * @returns {Number}
 */ function validate_solve_nums(times1, solve_nums) {
    // include all solves
    if (solve_nums === undefined) solve_nums = times1.length;
    solve_nums = parseFloat(solve_nums);
    if (!Number.isFinite(solve_nums)) solve_nums = times1.length;
    if (solve_nums <= 0) solve_nums = 1;
    if (solve_nums >= times1.length) solve_nums = times1.length;
    solve_nums = times1.length - solve_nums;
    return solve_nums;
}
/**
 ...
 */ function validate_average_list(average_list) {
    // default value
    if (average_list === undefined) average_list = [
        5,
        12,
        100,
        1000,
    ...
    for(let i = 0; i < average_list.length; i++){
        let num1 = average_list[i];
        // if num is an integer, add to dict
        num1 = parseInt(num1);
        if (Number.isInteger(num1)) average_dict[num1] = [];
    }
    return average_dict;
}
/**
 ...
 */ function validate_streaks(streaks) {
    ...
    for(let i = 0; i < streaks.length; i++){
        ...
        if (Number.isFinite(num1)) streak_dict[num1] = [
            0,
            0
        ];
    }
 ...
 */ function total_time_solving_text(total_time_solving) {
    let text;
    // Time calculations
    days = Math.floor(total_time_solving / 86400);
    hours = Math.floor(total_time_solving % 86400 / 3600);
    minutes = Math.floor(total_time_solving % 3600 / 60);
 ...
 */ function print_stats(file_or_text, file_flag, histogram_flag, dot_flag, average_list, graph_min, graph_max, min, max, solve_nums, zero_flag, step, streaks) {
    ...
    let times1;
    if (file_flag) times1 = csv_parse(file_or_text);
    else times1 = text_parse(file_or_text);
    if (times1.length <= 0) {
    ...
    if (label != null || label != undefined) label.parentNode.removeChild(label);
    // input validation
    // one liners
    ...
    solve_nums = validate_solve_nums(times1, solve_nums);
    times1 = times1.slice(solve_nums, times1.length + 1);
    let average_dict = validate_average_list(average_list);
    naive_numtimes = times_to_time_nums(times1);
    // naive sort for min/max
    // sort by value
    let naive_sort = naive_numtimes.sort(function(a, b) {
        return a - b;
    });
    ...
    best_time = times1[0].getNum;
    worst_time = times1[0].getNum;
    let numtimes = [];
    for(let index = 0; index < times1.length; index++){
        let time1 = times1[index];
        total++;
        if (time1.getPlusTwo) num_plus_two++;
        for(const key in average_dict)if (index > key - 1) {
            let sub_times = [];
            for(let n = index - (key - 1); n < index + 1; n++)sub_times.push(times1[n]);
            let avg = average_of_n(sub_times);
            if (avg != null) average_dict[key].push(avg);
        }
        let num1 = time1.getNum;
        if (isNaN(num1)) {
            // should never be true--but if it were it would mess stuff up
            console.error("Solve " + index + " was NaN.");
            continue;
        }
        total_time_solving += num1;
        if (time1.getDNF) {
            num_dnf++;
            // streak counting
        ...
        if (num1 < best_time) best_time = num1;
        if (num1 > worst_time) worst_time = num1;
        //streak counting
        for(const streak in streak_dict){
            let streak_list = streak_dict[streak];
            let cur_streak = streak_list[0];
            ...
            if (num1 < streak_num) cur_streak++;
            if (num1 >= streak_num) cur_streak = 0;
            if (cur_streak > best_streak) best_streak = cur_streak;
            // update streak
            streak_dict[streak][0] = cur_streak;
            streak_dict[streak][1] = best_streak;
...
function main() {
    ...
    while(averages.length > 0)averages[0].parentNode.removeChild(averages[0]);
    // remove streak divs
    let streaks = document.getElementsByClassName("streak");
    while(streaks.length > 0)streaks[0].parentNode.removeChild(streaks[0]);
    let user_input = text_input_div.value;
    print_stats(user_input, false);
}
text_input_div.addEventListener("input", main);

//# sourceMappingURL=index.a2bee3ad.js.map
